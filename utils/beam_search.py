# Original source: https://github.com/mikecvet/beam
...
class GeneratedSequence:
    ...
    def decode_tokens(self):
        return self.tokenizer.decode_by_ids(self.ids())
    ...
    def __eq__(self, other_sequence):
        return (
            self.normalized_score - other_sequence.normalized_score <= 1e-5
            and self.ids() == other_sequence.ids()
        )

    def __ne__(self, other_sequence):
        return (
            self.normalized_score - other_sequence.normalized_score > 1e-5
            or self.ids() != other_sequence.ids()
        )
